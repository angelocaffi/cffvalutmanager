...
/// <summary>Fido2Configuration values shared by every WebAuthn test — must match what FakeWebAuthnAuthenticator signs for.</summary>
internal static class WebAuthnTestConfig
{
    public const string RpId = "localhost";
    public const string Origin = "https://localhost:5001";

    public static IFido2 CreateFido2() => new Fido2(new Fido2Configuration
    {
        ServerDomain = RpId,
        ServerName = "Test",
...
/// attestation/assertion responses (ECDSA P-256, "none" attestation format) so
/// WebAuthnService's actual Fido2NetLib integration gets exercised end-to-end, not just mocked
/// around. There is no browser in a unit test to drive <c>navigator.credentials</c>, so this
/// stands in for one — the same technique Fido2NetLib's own test suite uses.
/// </summary>
internal sealed class FakeWebAuthnAuthenticator
{
    ...
    /// that supports the WebAuthn PRF extension and evaluated it — same JSON shape as the real
    /// <c>credential.getClientExtensionResults().prf.results.first</c> (see
