...
**Fix di sicurezza (penetration test, 2026-08-20, finding #1 — vedi [pentest-report-2026-08-20.md](pentest-report-2026-08-20.md))**: nonostante quanto appena descritto, `POST /api/tenants` era rimasto raggiungibile pubblicamente senza alcuna autenticazione anche dopo l'introduzione del gate — bypassando completamente la verifica email che questo stesso paragrafo descrive come motivazione del gate. Corretto richiedendo un chiamante autenticato con ruolo SuperAdmin quando l'host gira in ambiente Production (`AuthEndpoints.cs`); rimane anonimo fuori da Production, perché è anche il primitivo di bootstrap usato da tutta la test suite (che non ha ancora nessun utente da autenticare quando lo invoca). Collegato: la stessa verifica ha trovato che il provisioning non era resiliente a un fallimento dell'invio email (dominio non recapitabile → eccezione non gestita, `500` al chiamante nonostante Tenant/Admin/Vault fossero già stati creati con successo) — l'invio email era già correttamente "best-effort, dopo il commit" per design, ma l'eccezione non veniva catturata; ora viene loggata e non fa più fallire la richiesta (`ProvisionTenantService.cs`).
