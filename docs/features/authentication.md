...
**Disabilitazione TOTP da `/security`** — a differenza dell'Email OTP, il TOTP non aveva un percorso self-service per disattivarlo una volta confermato (`IMfaSetupService` esponeva solo `SetupTotpAsync`/`ConfirmTotpAsync`). Aggiunto `DisableTotpAsync` (nuovo endpoint `POST /api/auth/mfa/totp/disable`, nuovo `AuditAction.MfaDisabled`, notifica di sicurezza via `ISecurityNotificationService` come per gli altri fattori): azzera `MfaEnabled`/`MfaSecret` senza tentare di decifrare il secret esistente, quindi funziona anche se il secret non è più decifrabile (vedi [../deployment.md](../deployment.md) sulla persistenza del key ring di Data Protection — è esattamente lo scenario che ha reso necessario questo bottone, un account rimasto bloccato fuori dal login senza modo di resettare da UI). Riattivare richiede sempre un nuovo `SetupTotpAsync`/QR code, mai il riuso di un secret precedente.
