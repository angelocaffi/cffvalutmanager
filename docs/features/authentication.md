...
19 nuovi test (10 Infrastructure incluso il round-trip crittografico reale + 5 Api; 358 in totale nella solution). Manca ancora: le pagine Blazor (`Web.Client`) per registrare/gestire i dispositivi e per il pulsante "Accedi con biometria" nella schermata di login (il vincolo di riservare lo spazio in UI era già stato rispettato in Fase 1, ma il pulsante stesso e il flusso di asserzione lato client non sono ancora collegati).
