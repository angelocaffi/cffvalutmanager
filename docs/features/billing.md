...
## Prezzo modificabile da SuperAdmin (promozioni)

A differenza del prezzo VIP (una singola coppia email/prezzo da configurazione server, comp per un solo account), questo è il prezzo **pubblico** pagato da chiunque, modificabile a runtime da un SuperAdmin senza redeploy — pensato per promozioni a tempo (es. "offerta di lancio, -40% per 7 giorni").

**Modello dati**: nuova entità `BillingPricing` (`src/CffVaultManager.Domain/Entities/BillingPricing.cs`), riga singola identificata da un GUID fisso (`BillingPricing.SingletonId`) — non tenant-scoped (nessun query filter, nessuna FK a `Tenant`), stessa ragione di `TenantProvisioningRequest`: è configurazione di piattaforma, non dato di un tenant. Campi: `StandardAnnualPrice` (obbligatorio, > 0), `DiscountedAnnualPrice`/`DiscountExpiresAt` (opzionali ma **sempre una coppia**: uno sconto senza scadenza non è una promozione, una scadenza senza sconto non ha senso — l'entità rifiuta l'uno senza l'altro), `PromoMessage` (opzionale, max 280 caratteri, mostrato agli utenti). Se non esiste ancora nessuna riga (installazione nuova, nessun SuperAdmin ha mai toccato il prezzo), `BillingService`/`BillingPricingAdminService` ricadono su `Billing:AnnualPrice` da configurazione — comportamento identico a prima di questa feature, nessuna migrazione dati necessaria per gli ambienti esistenti.

**Precedenza di prezzo** in `BillingService.ResolvePriceAsync` (dal più specifico al meno specifico): (1) prezzo VIP, se configurato e l'email del chiamante combacia; (2) prezzo scontato, se `BillingPricing` esiste e lo sconto non è scaduto (`BillingPricing.IsDiscountActive`); (3) prezzo standard da `BillingPricing`, se la riga esiste; (4) `Billing:AnnualPrice` da configurazione, fallback finale. Un VIP paga sempre il suo prezzo comp anche durante una promozione pubblica attiva — sono due meccanismi indipendenti, non si sommano.

**Superficie SuperAdmin**: `IBillingPricingAdminService`/`BillingPricingAdminService` (separato da `IBillingService`, che resta la superficie tenant-facing per checkout/status) — `GET/PUT /api/admin/billing/pricing`, sotto lo stesso gruppo `RequireAuthorization(Roles = "SuperAdmin")` di `/api/admin/tenants/*` in `AdminEndpoints.cs`. Ogni aggiornamento scrive un `AuditLogEntry` con `TenantId = null` (evento di piattaforma, non di un tenant specifico — lo stesso campo esiste già nullable proprio per questo caso, vedi `AuditLogEntry`), nuovo `AuditAction.BillingPricingUpdated`. Validazione (prezzo positivo, sconto più basso dello standard, scadenza nel futuro, messaggio ≤ 280 caratteri) vive nell'entità stessa (`BillingPricing.Update`), non duplicata nell'endpoint — un `ArgumentException` diventa un 400 con il messaggio d'errore.

**Superficie utente**: `GET /api/billing/status` (già esistente, invariato nell'endpoint) ora espone anche `StandardAnnualPrice`/`EffectivePrice`/`Currency`/`PromoMessage`/`PromoExpiresAt` — `EffectivePrice` è ciò che l'utente autenticato pagherebbe davvero (VIP già applicato), gli altri campi servono solo a disegnare un banner "prima/dopo" quando una promozione è attiva, mai a decidere l'importo lato client (l'invariante "l'importo non è mai un input del client" resta invariata: `CreateCheckoutAsync` ricalcola tutto server-side). `Billing.razor` mostra il banner solo quando `EffectivePrice < StandardAnnualPrice`. Nuova pagina `Web.Client/Pages/AdminBilling.razor` (`/admin/billing`, `[Authorize(Roles = "SuperAdmin")]`, link "Prezzi" in `MainLayout.razor` accanto ad "Amministrazione") per impostare i quattro campi.

**Test**: nuovi/estesi in Infrastructure (validazione pura dell'entità, `BillingPricingAdminService` get/update/audit, precedenza di prezzo in `BillingService` incluso il caso sconto scaduto e VIP-sopra-sconto) e in Api (403 per un Admin di tenant, 401 senza token, round-trip GET/PUT, 400 su input non valido). 596 test totali nella solution.
