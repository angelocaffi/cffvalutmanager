...
- Pagine Blazor (`Web.Client`): `Shared/CryptoWalletFields.razor` (dentro `VaultItemDetail.razor`) supporta più indirizzi per voce (aggiungi/rimuovi), riconoscimento rete automatico dal primo indirizzo non vuoto, validazione di plausibilità live su indirizzo/mnemonic con avviso visivo. Reveal di chiave privata/seed phrase: stesso pattern di conferma esplicita + finestra di validità breve (auto-nascondimento dopo 15s) e copia con auto-clear appunti usato per numero carta/CVV, entrambe le azioni tracciate come evento `Revealed`. Verificato dal vivo in un browser reale con un vero round-trip di cifratura/decifratura. Resta deliberatamente non implementata la validazione crittografica completa (vedi punto sopra).
