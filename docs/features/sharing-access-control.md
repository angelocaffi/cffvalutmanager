...
> Stato: **parzialmente promossa a MVP** dal modello multi-tenant — vedi nota sotto. Il caso base "vault di organizzazione condiviso tra i membri di un tenant" è necessario da subito; condivisioni ad-hoc tra singoli utenti restano backlog v2. Fase 4 (condivisione granulare di singola voce) **completa**: link di condivisione esterna, generazione keypair, backend e interfaccia Blazor della condivisione live tra utenti dello stesso tenant sono tutti implementati e verificati dal vivo — vedi le sezioni dedicate più sotto. Ruoli fini (`Owner`) sui vault di organizzazione **completi** lato API — vedi sotto. Resta solo la UI per i vault di organizzazione (creazione/gestione membership), non collegata alla condivisione di singola voce.
...
- **In scope Fase 1**: un Admin può creare un vault di organizzazione e invitare membri del proprio tenant (Operator/Admin) con permesso lettura/modifica/proprietario, usando lo schema a chiave asimmetrica descritto sotto.
- **Backlog v2**: condivisione granulare di singole voci tra utenti arbitrari (**completata in Fase 4**, non più backlog), condivisione cross-tenant (che comunque non deve mai essere possibile, per definizione di tenant isolation).
...
- Vault "di gruppo" con più proprietari/membri e ruoli: `Read`, `ReadWrite`, `Owner` (quest'ultimo abilita anche l'invito/revoca di altri membri — vedi sotto).
...
**Modello dati**: nuova entità `VaultMembership` [tenant-scoped] — `VaultId`, `UserId`, `Permission` (`Read`/`ReadWrite`/`Owner`), `WrappedVaultDek`, `EphemeralPublicKey`, `InvitedByUserId`, `CreatedAt`, `RevokedAt` (nullable — la riga resta per audit anche dopo la revoca). Indice univoco filtrato `(TenantId, VaultId, UserId) WHERE RevokedAt IS NULL`: al più una membership attiva per utente per vault.

**Controllo accessi**: `VaultAccessGuard.GetAccessibleVaultAsync` sostituisce (per `VaultItemService`, `FolderService`, `TagService`) il precedente controllo solo-proprietario — un vault personale resta ownership-only (`ReadWrite` implicito per il proprietario), un vault di organizzazione richiede una membership attiva e restituisce il permesso di quella membership. Le operazioni di scrittura verificano `VaultPermissionExtensions.CanWrite()` (vero per `ReadWrite` e `Owner`) e lanciano `InsufficientVaultPermissionException` (→ `403`) altrimenti; qualunque mancanza di accesso (vault inesistente, membership assente o revocata) è sempre "not found" (`404`), mai "forbidden", per non rivelare l'esistenza del vault a chi non ne fa parte.

**Ruoli fini (`Owner`)**: `Owner` include tutte le capacità di `ReadWrite` (voci, cartelle, tag, link esterni, promozione a condivisione singola) più l'autorità di invitare/revocare membri di *questo* vault (`VaultMembershipService.InviteAsync`/`RevokeAsync` richiedono `Permission == Owner`, non più `ReadWrite`). Questa autorità è interamente vault-scoped e disaccoppiata dal ruolo tenant-wide dell'utente: gli endpoint di invito/revoca richiedono solo autenticazione (`.RequireAuthorization()`), il controllo reale vive nel servizio — un Admin del tenant con solo `ReadWrite` su un vault non può gestirne i membri, mentre un Operator che ne è `Owner` può, esattamente come già avviene per `ItemSharePermission.Owner` nella condivisione di singola voce. Non esiste un vincolo di Owner singolo per vault (più persone possono esserlo) né un endpoint per cambiare il permesso di un membro esistente: per cambiarlo si revoca e si re-invita, stessa disciplina già scelta per `ItemMembership`. Il creatore di un vault di organizzazione riceve `Owner` (non più solo `ReadWrite`) fin dalla creazione.

**Endpoint**: `POST/GET /api/vaults/organization` (creazione ed elenco vault di organizzazione accessibili, creazione Admin-only), `POST /api/vaults/{vaultId}/memberships` (invito, richiede `Owner` sul vault), `POST /api/vaults/{vaultId}/memberships/{userId}/revoke` (revoca, richiede `Owner` sul vault), `GET /api/vaults/{vaultId}/memberships` (elenco membri, qualunque membro attivo), `GET /api/tenant/users/{userId}/public-key` (per il client che deve cifrare la DEK per un nuovo invitato — mai cross-tenant).
...
62 test (12 `CffVaultManager.Crypto.Tests` per lo scambio di chiavi, 32 `CffVaultManager.Infrastructure.Tests`, 18 `CffVaultManager.Api.Tests` end-to-end). Il ruolo `Owner` (vedi sopra) ha aggiunto 5 nuovi test (4 Infrastructure + 1 Api end-to-end; 464 in totale nella solution), incluso un test che dimostra il punto centrale della feature — un Operator invitato come `Owner` gestisce la membership del vault senza il ruolo tenant Admin.
...
Da fare: pagine Blazor (`Web.Client`) per creare/gestire vault di organizzazione e membership (la generazione della coppia di chiavi lato client, prerequisito bloccante, è ora implementata — vedi sotto).
...
**Interfaccia Blazor (implementata)**:

- `Shared/ItemKeyResolver.cs` — servizio che sblocca la chiave-voce del chiamante: recupera la propria coppia di chiavi tramite `GET /api/auth/keypair` (nuovo — Slice 2 copriva solo il caricamento della chiave, non il recupero successivo necessario per sbloccare una condivisione: la chiave privata cifrata viene decifrata al volo con la DEK di sessione a ogni utilizzo, mai mantenuta in memoria oltre la singola operazione), poi esegue `UnwrapKey`/`WrapKey` tramite `IAsymmetricKeyExchangeService` (registrato in `Web.Client` per la prima volta in questa fase).
- `VaultItemDetail.razor` (il proprietario): nuova card "Condivisione" — bottone "Condividi con un utente" (email + permesso Viewer/Editor), elenco membri con revoca (solo Owner). Il percorso di decifratura/cifratura esistente ora controlla `MySharedAccess`: se presente usa la chiave-voce sbloccata via `ItemKeyResolver`, altrimenti la DEK del vault come sempre. La stessa promozione (prima condivisione: genera la chiave, ricifra il payload, wrappa per sé stesso e per il destinatario) e la stessa rotazione-alla-revoca (rewrap per tutti i membri rimanenti) già viste lato server sono ora orchestrate dal client.
- `Pages/SharedItemDetail.razor` (`/shared-items/{itemId}`) e `Pages/SharedWithMe.razor` (`/shared-with-me`) — vista dedicata per un destinatario che **non ha accesso al vault** che contiene la voce: nessun `vaultId` in gioco, risolta solo tramite la propria `ItemMembership`. Sola lettura per i Viewer (nessun bottone "Salva"), modificabile per gli Editor.
- **Correzione scoperta solo dal vivo**: `VaultItems.razor`/`VaultTrash.razor` (le liste che decifrano titoli per la vista d'insieme) non conoscevano `MySharedAccess` e mostravano "(non decifrabile)" per una voce del *proprio* vault una volta promossa — bug invisibile nei test (che non coprono la decifratura del titolo in lista) e nella pagina di dettaglio (che già gestiva correttamente il branch). Corretto propagando lo stesso branch chiave-voce/DEK anche lì.

Verificato dal vivo con **due utenti reali nello stesso tenant** (credenziali Argon2id/AES-256-GCM reali, non mockate, provisionate via un piccolo programma seed che referenzia `CffVaultManager.Crypto` direttamente): condivisione di una voce Password, destinatario Viewer che vede correttamente titolo/username/password decifrati, nessun bottone "Salva" per il Viewer, **il proprietario modifica la password e il destinatario la vede aggiornata alla navigazione successiva (conferma della condivisione live, non uno snapshot)**, revoca che rimuove davvero l'accesso (la voce sparisce dal feed "condivisi con me" del destinatario). Confermato anche che un vero refresh di pagina (non una navigazione SPA) fa perdere la sessione per design di questo progetto — comportamento atteso, non un bug della condivisione.

7 nuovi test backend (5 Infrastructure + 2 Api, per `GET /api/auth/keypair` — il prerequisito mancante scoperto costruendo questa UI; 449 in totale nella solution). Nessun nuovo test automatico per la UI stessa (lavoro solo `Web.Client`).
