...
(the Blazor WASM host), PostgreSQL, and Caddy (TLS termination) wired together by
...
This project never auto-migrates on startup (see CLAUDE.md). `postgres`'s port `5432` is
published to the host as `15432` by `docker-compose.yml` (not `5432` — a machine with its own
local Postgres instance already owns that port, same defensive reasoning previously applied to
SQL Server's `15433`). Apply migrations from your machine (or from the deployment host itself)
against `15432`:
...
dotnet ef database update \
  ...
  --connection "Host=localhost;Port=15432;Database=CffVaultManager;Username=postgres;Password=<POSTGRES_PASSWORD from .env>"
```
...
control, real PostgreSQL, no managed-database dependency):
...
2. **Always Free Compute VM** (you do this, in the OCI console): create an instance using shape
   ...
   shareable across up to 4 such instances), Ubuntu 24.04 or Oracle Linux 9 image (either works;
   only the package-manager commands below differ, apt vs dnf), and attach a **Reserved Public
...
3. **Open the firewall** (you do this, in the VM's attached Security List / Network Security
   ...
   default already). The VM's own OS firewall needs the same two ports allowed: Ubuntu's `ufw` if
   enabled, or Oracle Linux's `firewalld` (`sudo firewall-cmd --permanent --add-service=http
...
5. **On the VM** (SSH in, then): install Docker Engine + the Compose plugin (Docker's official
   `get-docker.sh` convenience script on Ubuntu, or the Docker CE `dnf` repo on Oracle
   Linux/RHEL-family), clone this repository (or copy `docker-compose.yml`, `Caddyfile`, `.env`,
   and `src/` over), fill in `.env` with
   `PUBLIC_DOMAIN`/`CADDY_ACME_EMAIL`/a strong `JWT_SIGNING_KEY`/`POSTGRES_PASSWORD`, edit
   `wwwroot/appsettings.Production.json` as above, then:
   ```
   docker compose up --build -d
   ```
   followed by the migration step above (run from the VM itself against `localhost,15432`).
...
- PostgreSQL data persists in the `postgres-data` named volume; back it up like any production
  database.
